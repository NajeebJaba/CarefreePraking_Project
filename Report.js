...
const Report = ({navigation,route}) => {
  ...
  const addItem=()=>{
   ...
   // alert(value)
    ...
    fetch(`${global.ip.key}:3000/report/`,{
        method:'post',
        headers:{
      ...
      .then(res=>{
        if(res.data){
            res.data.forEach(item=>{
                ...
                arr.push({_id:item._id,Name:item.Name,Vehicle:item.Vehicle,Phone:item.Phone,Paking:item.Paking,Date:item.DatePaking});
            })
...
<Pressable onPress={()=>{navigation.replace('Admin',{})}} style={{position:'absolute',left:10,top:10,width:40,height:40,zIndex:1}}>
    ...
    Entry and exits report parking
    </Text>
